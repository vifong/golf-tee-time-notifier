...
def writeMessage(course, target_date, results):
    # message = "Tee Time Alert! {results}".format(results=results)
    message = "Tee Time Alert! {course_name} - {date} {results}".format(
        course_name=course['name'], date=target_date, results=results)
    print(message)
